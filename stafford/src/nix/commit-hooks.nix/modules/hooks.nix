...
in {
  options.settings = {
    ...
    nix-linter = {
      checks = mkOption {
        type = types.listOf types.str;
        description = "Available checks (See `nix-linter --help-for [CHECK]` for more details)";
        default = [];
      };
    };
    prettier = {
      binPath = mkOption {
        type = types.path;
        description = "Prettier binary path. E.g. if you want to use the prettier in node_modules, use ./node_modules/.bin/prettier";
  ...
  config.hooks = {
    ...
    ormolu = {
      ...
      entry = "${tools.ormolu}/bin/ormolu --mode inplace ${
        lib.escapeShellArgs (lib.concatMap (ext: ["--ghc-opt" "-X${ext}"])
    ...
    fourmolu = {
      name = "fourmolu";
      description = "Haskell code prettifier.";
      entry = "${tools.fourmolu}/bin/fourmolu --mode inplace ${
        lib.escapeShellArgs (lib.concatMap (ext: ["--ghc-opt" "-X${ext}"])
          settings.ormolu.defaultExtensions)
      }";
      files = "\\.l?hs$";
    ...
    nix-linter = {
      ...
      entry = "${tools.nix-linter}/bin/nix-linter ${
        lib.escapeShellArgs (lib.concatMap (check: ["-W" "${check}"])
    ...
    sfrc-fq-ref = {
      name = "sfrc-fq-ref";
      description = "verify fastq files are referenced exactly once";
      entry = "${tools.fqref}";
      pass_filenames = false;
    };
    sfrc-germline = {
      name = "sfrc-germline";
      description = "verify germline expressions instantiate";
      entry = "${tools.nix}/bin/nix-instantiate src/bionix/sfrc01243.nix";
      files = "\\.nix$";
      pass_filenames = false;
    };
    sfrc-multisample = {
      name = "sfrc-multisample";
      description = "verify multisample expressions instantiate";
      entry = "${tools.nix}/bin/nix-instantiate src/bionix/sfrc01056.nix";
      files = "\\.nix$";
      pass_filenames = false;
    };
    sfrc-inst = {
      name = "sfrc-instantiate";
      description = "verify SFRC expressions instantiate";
      entry = "${tools.nix}/bin/nix-instantiate";
      files = "sfrc[0-9][0-9][0-9][0-9][0-9]\\.nix";
    };
  };
}
